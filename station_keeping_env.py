...
class MagicBallEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        high = np.array([np.finfo(np.float32).max]*17)
        self.observation_space = gym.spaces.Box(-high, high, shape=(17,), dtype=np.float32)
    ...
    def _get_obs(self):
        ...
        return np.concatenate(
            [
                ...
                self.data.sensordata[19:21],     # current location
    ...
    def step(self, action):
        ...
        angular_vel_penalty = - 0.001 * np.sum(np.square(self.data.sensordata[9:12]))
        ...
        position_reward = 1 * (1 - np.tanh(np.linalg.norm([self.data.sensordata[19],self.data.sensordata[20],self.rpy[2]])))
